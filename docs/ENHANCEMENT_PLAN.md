...
| **P2** | Medium | **Version compatibility matrix** | Document which FutuOpenD versions have been tested. Pin the version in CI (`ARG FUTU_OPEND_VER=10.3.6308`) and verify it still downloads from Futu's CDN in CI. Currently tested: 10.3.6308, 10.2.6208. | Futu updates their binary URL structure. Outdated links break builds silently. |
