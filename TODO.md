...
- [ ] **Both build stages always run** — `docker build --target final-ubuntu` still executes the Rocky stage. Restructure Dockerfile so only the relevant stage runs, cutting build time in half.
...
- [ ] **Parallelize builds in `dockerbuild.sh`** — Ubuntu and Rocky build sequentially. They could run in parallel with `&` + `wait`.
